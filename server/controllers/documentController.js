...
export const scanDocument = (req, res) => {
  ...
  db.get(query, [userId], (err, user) => {
    if (err) {
      ...
      return res.status(500).json({ message: 'Error checking credits', error: err });
    }
    ...
    // Deduct 1 credit
    const deductCreditsQuery = 'UPDATE users SET credits = credits - 1 WHERE id = ?';
    db.run(deductCreditsQuery, [userId], (err) => {
      if (err) {
        console.error('Error deducting credits:', err);
        return res.status(500).json({ message: 'Error deducting credits', error: err });
      }
      ...
      extractTextFromFile(filepath)
        .then((content) => {
          ...
          const insertQuery = 'INSERT INTO documents (user_id, filename, filepath, content) VALUES (?, ?, ?, ?)';
          db.run(insertQuery, [userId, file.originalname, filepath, content], function (err) {
            if (err) {
            ...
            const insertScanHistoryQuery = 'INSERT INTO scan_history (user_id, document_id, matched_docs) VALUES (?, ?, ?)';
            db.run(insertScanHistoryQuery, [userId, this.lastID, ''], (err) => {
              if (err) {
                console.error('Error logging scan history:', err);
                return res.status(500).json({ message: 'Error logging scan history', error: err });
              }

              return res.status(200).json({ message: 'Document uploaded and scanned successfully' });
            });
          });
        })
        .catch((err) => {
          console.error('Error extracting text:', err);
          res.status(500).json({ message: 'Error extracting text from document', error: err });
        });
    });
  });
};
    ...
    db.run(deductCreditsQuery, [userId], (err) => {
      ...
      db.get(getDocumentQuery, [docId], (err, targetDocument) => {
        if (err || !targetDocument) {
          return res.status(400).json({ message: 'Target document not found' });
        }

        const targetContent = targetDocument.content;

        // Fetch all documents from the database
        const getAllDocsQuery = 'SELECT id, filename, filepath, content FROM documents';
        db.all(getAllDocsQuery, [], (err, documents) => {
          if (err) {
            return res.status(500).json({ message: 'Error fetching documents from database', error: err });
          }
          ...
          const similarDocuments = documents
            ...
            .map(doc => {
              const similarity = calculateSimilarity(targetContent, doc.content);
              if (similarity >= 0.5) { 
                const formattedFilePath = `/${doc.filepath.replace(/\\/g, '/')}`; // Only include documents with at least 50% similarity
                return {
                  docId: doc.id,
          ...
          if (similarDocuments.length === 0) {
            return res.status(200).json({ message: 'No similar documents found' });
          }

          // Log the scan activity in the scan_history table for each comparison
          const matchedDocs = similarDocuments.map(doc => doc.filename).join(', ');
          const insertScanHistoryQuery = 'INSERT INTO scan_history (user_id, document_id, matched_docs) VALUES (?, ?, ?)';
          db.run(insertScanHistoryQuery, [req.user.id, docId, matchedDocs], (err) => {
            if (err) {
              console.error('Error logging scan history:', err);
            }

            // Update admin analytics (total scans)
            const updateAnalyticsQuery = 'UPDATE admin_analytics SET total_scans = total_scans + 1 WHERE id = 1';
...
export const getUserDocuments = (req, res) => {
  ...
  db.all(query, [userId], (err, rows) => {
    if (err) {
      ...
      return res.status(500).json({ message: 'Error fetching user documents', error: err });
    }
